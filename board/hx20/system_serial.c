...
int write_slot(int address){
    ...
    for (i = 0; i < sizeof(serials_info); i+= 256) {
        spi_flash_write(address + i, MIN(256, i - sizeof(serials_info)), (void*)(&serials_info) + i);
    }
...
int read_slot(int address){
    int i;
    for (i = 0; i < sizeof(serials_info); i+= 256) {
        spi_flash_read((void*)(&serials_info) + i, address + i, MIN(256, i - sizeof(serials_info)));
    }
    return EC_SUCCESS;
}
...
int initialize_structure(void) {
    /*
    uint8_t data[256];
    struct ec_flash_serial_info * serialinfo;
    crc_t crc;
*/
    crc_t crc;
...
int print_serial(int address) {
    ...
    CPRINTS("    CRC: 0x%04x %s", serials_info.crc, update_number == -1? "Invalid": "Valid");
    if (update_number  != -1) {
        for (i = 0; i < SN_MAX; i++) {
