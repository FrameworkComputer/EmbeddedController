...
K_MUTEX_DEFINE(sb_rmi_mutex);

#define CLEAR_MUTEX_RETURN_ERROR(fn)	\
	do {				\
		int err = (fn);		\
		if (err != EC_SUCCESS)	\
			goto error;	\
...
int sb_rmi_mailbox_xfer(int cmd, uint32_t msg_in, uint32_t *msg_out_ptr)
{
	 ...
	 *         SBRMIx40[Software Interrupt] is cleared
	 */
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_write(SB_RMI_IN_BND_MSG7_REG, 0x80));
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_assert_interrupt(0));

	/* Step 2: writes the command to SBRMI::InBndMsg_inst0 (SBRMI_x38) */
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_write(SB_RMI_IN_BND_MSG0_REG, cmd));
	/* Step 3: msgIn to {SBRMI_x3C(MSB):SBRMI_x39(LSB)} */
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_write(SB_RMI_IN_BND_MSG1_REG, msg_in & 0xFF));
	CLEAR_MUTEX_RETURN_ERROR(
		sb_rmi_write(SB_RMI_IN_BND_MSG2_REG, (msg_in >> 8) & 0xFF));
	CLEAR_MUTEX_RETURN_ERROR(
		sb_rmi_write(SB_RMI_IN_BND_MSG3_REG, (msg_in >> 16) & 0xFF));
	CLEAR_MUTEX_RETURN_ERROR(
	 ...
	 *         firmware to start service.
	 */
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_assert_interrupt(1));
	...
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_read(SB_RMI_OUT_BND_MSG0_REG, &val));
	if (val != cmd) {
		/**
		 * Don't spam the EC logs when the AP is hung. Instead, log the
		 * first few failures, and then indicate the AP is likely hung.
	...
	/* Step 8: read msgOut from {SBRMI_x34(MSB):SBRMI_x31(LSB)} */
	*msg_out_ptr = 0;
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_read(SB_RMI_OUT_BND_MSG1_REG, &val));
	*msg_out_ptr |= val;
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_read(SB_RMI_OUT_BND_MSG2_REG, &val));
	*msg_out_ptr |= val << 8;
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_read(SB_RMI_OUT_BND_MSG3_REG, &val));
	*msg_out_ptr |= val << 16;
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_read(SB_RMI_OUT_BND_MSG4_REG, &val));
	*msg_out_ptr |= val << 24;
	 ...
	 *          ALERT upon completion of the current mailbox command.
	 */
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_write(SB_RMI_STATUS_REG, 0x2));

	/* Step 10: read the return code from OutBndMsg_inst7 (SBRMI_x37) */
	CLEAR_MUTEX_RETURN_ERROR(sb_rmi_read(SB_RMI_OUT_BND_MSG7_REG, &val));
...
error:
	mutex_unlock(&sb_rmi_mutex);
	if (ret)
		CPRINTS("SB-RMI Error: %d", ret);
