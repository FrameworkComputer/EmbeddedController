 ...
 * using XIP mode. Such chips use SPI for fetching instructions from flash.
 * There is no flash controller that introduces additional protection features.
 * From hardware perspective, the only available flash protection mechanism is
 * the one provided by a connected flash chip.
 *
...
static int flash_andes_xip_lock_status(const struct device *dev, bool enable)
{
	struct cros_flash_andestech_xip_data *data = DRV_DATA(dev);
	...
	return flash_ex_op(data->flash_dev, FLASH_ANDES_XIP_EX_OP_LOCK,
			   (uintptr_t)&op_in, 0);
}

static int flash_andes_xip_get_lock_status(const struct device *dev,
					   bool *state)
{
	int ret;
	...
	ret = flash_ex_op(data->flash_dev, FLASH_ANDES_XIP_EX_OP_LOCK_STATE,
			  (uintptr_t)NULL, &op_out);
	if (!ret) {
		*state = op_out.state;
	}
...
static int cros_flash_andes_xip_get_status(const struct device *dev,
					   uint8_t *sr1, uint8_t *sr2)
{
	struct andes_xip_ex_ops_get_out op_out;
	int ret;

	ret = flash_andes_xip_get_status_regs(dev, &op_out);
	if (!ret) {
		*sr1 = op_out.regs[0];
		*sr2 = op_out.regs[1];
	}
...
static uint32_t cros_flash_andes_xip_get_protect_flags(const struct device *dev)
{
	...
	ret = flash_andes_xip_get_lock_status(dev, &status_locked);
	if (ret) {
						...
						uint32_t new_flags)
{
	...
	if (status_locked) {
		int ret2;
		ret2 = flash_andes_xip_lock_status(dev, true);
		if (ret2) {
...
static int cros_flash_andes_xip_write(const struct device *dev, int offset,
...
static int cros_flash_andes_xip_erase(const struct device *dev, int offset,
				      int size)
{
	struct cros_flash_andestech_xip_data *data = DRV_DATA(dev);
	int ret = 0;
...
static int flash_andes_xip_init(const struct device *dev)
{
	struct cros_flash_andestech_xip_data *data = DRV_DATA(dev);
...
BUILD_ASSERT(CONFIG_FLASH_ANDES_QSPI_INIT_PRIORITY <
	     CONFIG_CROS_FLASH_INIT_PRIORITY);

static struct cros_flash_andestech_xip_data cros_flash_data;
DEVICE_DT_INST_DEFINE(0, flash_andes_xip_init, NULL, &cros_flash_data, NULL,
