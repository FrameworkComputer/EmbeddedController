...
#define FPC1145_IMAGE_PARAM_INITIALIZER(idx, inst)                             \
	{                                                                      \
		.frame_size =                                                  \
...
#define FPC1145_BUILD_ASSERT_IMAGE_SIZE(idx, inst)                             \
	BUILD_ASSERT(                                                          \
...
#define FPC1145_DEFINE(inst)                                                         \
	...
	static const struct fpc1145_cfg fpc1145_cfg_##inst = {                       \
		...
		.sensor_image_configs = { LISTIFY(                                   \
			FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)),           \
			FPC1145_IMAGE_PARAM_INITIALIZER, (, ), inst) },              \
	};                                                                           \
	LISTIFY(FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)),                   \
		FPC1145_BUILD_ASSERT_IMAGE_SIZE, (;), inst)                          \
	BUILD_ASSERT(                                                                \
		FINGERPRINT_SENSOR_NUM_CONFIGS(DT_DRV_INST(inst)) <=                 \
			NUM_IMAGE_CAPTURE_TYPES,                                     \
		"FPC1145: Number of image configs exceeds NUM_IMAGE_CAPTURE_TYPES"); \
