...
/* C1nv chipset-specific configuration */
...
DECLARE_HOOK(HOOK_CHIPSET_STARTUP, board_chipset_startup_c1nv,
	     HOOK_PRIO_DEFAULT);

void board_chipset_shutdown_c1nv(void)
{
	/*
	 * C1nv HDMI power loadswitch has active discharge powered by pp5000.
	 * Delay disabling pp5000 to give the loadswitch time to discharge
	 * HDMI Power rails on C1nv.
	 */
...
DECLARE_HOOK(HOOK_CHIPSET_SHUTDOWN, board_chipset_shutdown_c1nv,
	     HOOK_PRIO_DEFAULT);
