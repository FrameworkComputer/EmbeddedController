...
LOG_MODULE_REGISTER(kinmen_usbc, LOG_LEVEL_INF);
...
int board_get_pdc_for_port(int port, const struct device **dev)
{
	...
	rv = cbi_get_board_version(&board_version);

	if (rv) {
		LOG_ERR("%s: Cannot read CBI board version: %d. PDC config unknown!",
			__func__, rv);
	...
	case 1:
		...
		case 1:
			*dev = DEVICE_DT_GET(DT_NODELABEL(pdc_power_p1));
			return 0;
		case 2:
			if (!probe_pdc_chip_on_daughter_board(DEVICE_DT_GET(
				    DT_NODELABEL(pdc_power_p2)))) {
				*dev = NULL;
			} else {
				*dev = DEVICE_DT_GET(
					DT_NODELABEL(pdc_power_p2));
	...
	LOG_ERR("%s: No entry for board version (0x%x) or port %d", __func__,
		board_version, port);
