...
void send_movement_packet(void)
{
	...
	if (timeout == AUX_BUFFER_FULL_RETRIES ||
		(*task_get_event_bitmap(TASK_ID_TOUCHPAD) & PS2MOUSE_EVT_AUX_DATA)) {
...
{
	aux_data = data;
	task_set_event(TASK_ID_TOUCHPAD, PS2MOUSE_EVT_AUX_DATA);
}
...
{
	task_set_event(TASK_ID_TOUCHPAD, PS2MOUSE_EVT_I2C_INTERRUPT);
 ...
 *    CMD     DATA   ID
 * W 0x0022 0x0233 0x0023 opcode 0x02 - GET_REPORT, report id  0x3 INPUT_MODE_TOUCH,
 ...
 *  W 0x0022 0x0336 0x0023 0x0004 0x0306 opcode 0x03 feature report set configuration
 *  register to mode touch
 *
 ...
 *		0x0022 0x0336 0x0023 0x0004 0x0006 which sets feature report id 6 to
 *		input mode mouse
 */
...
{
	if (disable)
		task_set_event(TASK_ID_TOUCHPAD, PS2MOUSE_EVT_HC_DISABLE);
	else
		task_set_event(TASK_ID_TOUCHPAD, PS2MOUSE_EVT_HC_ENABLE);
}
...
void read_touchpad_in_report(void)
{
	...
	/**
	 *Zephyr i2c shim does not support this
...
static int command_emumouse(int argc, const char **argv)
{
	...
	if (argc < 4) {
		CPRINTS("mouse state 0x%x data_report: 0x%x btn:0x%x",
				mouse_state, data_report_en, button_state);
