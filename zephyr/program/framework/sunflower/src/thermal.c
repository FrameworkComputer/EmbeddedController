...
/* top-skin-f75303 */
#define TEMP_TOPSKIN TEMP_SENSOR_ID(DT_NODELABEL(temp_sensor_top_skin))
#define TEMP_TOPSKIN_F F75303_SENSOR_ID(DT_NODELABEL(top_skin_f75303))

/* local-f75303 */
#define TEMP_LOCAL TEMP_SENSOR_ID(DT_NODELABEL(temp_sensor_local))
#define TEMP_LOCAL_F F75303_SENSOR_ID(DT_NODELABEL(local_f75303))
...
void board_override_fan_control(int fan, int *temp)
{
	...
	if (chipset_in_state(CHIPSET_STATE_ON)) {
		...
		if (log_thermal && fan == 0) {
			f75303_get_val_mk(TEMP_TOPSKIN_F, &temps_mk[0]);
			f75303_get_val_mk(TEMP_CPU_F, &temps_mk[1]);
			f75303_get_val_mk(TEMP_LOCAL_F, &temps_mk[2]);
			CPRINTS(
				"\tThrm\t%d\t%d\t%d\t"
				"\t%d\t%d\t"
				"\t%d\t"
