...
/* PujjogaTwin sub-board hardware configuration */
...
test_export_static enum pujjogatwin_sub_board_type pujjogatwin_cached_sb =
	PUJJOGATWIN_SB_UNKNOWN;
/*
...
enum pujjogatwin_sub_board_type pujjogatwin_get_sb_type(void)
{
	 ...
	 * Return cached value.
	 */
	if (pujjogatwin_cached_sb != PUJJOGATWIN_SB_UNKNOWN)
		return pujjogatwin_cached_sb;

	pujjogatwin_cached_sb = PUJJOGATWIN_SB_NONE; /* Defaults to none */
	...
	default:
		...
		break;
	case FW_SUB_BOARD_3:
		pujjogatwin_cached_sb = PUJJOGATWIN_SB_HDMI_A;
 ...
 */
static void pujjogatwin_subboard_config(void)
{
	enum pujjogatwin_sub_board_type sb = pujjogatwin_get_sb_type();
#if CONFIG_NISSA_BOARD_HAS_HDMI_SUPPORT
	static struct ap_power_ev_callback power_cb;
#endif

	BUILD_ASSERT(USB_PORT_ENABLE_COUNT == 1,
		     "PujjogaTwin assumes no more than 1 USB-A ports");
	/*
...
DECLARE_HOOK(HOOK_INIT, pujjogatwin_subboard_config, HOOK_PRIO_POST_FIRST);
