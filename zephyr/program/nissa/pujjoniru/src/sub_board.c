...
/* Pujjoniru sub-board hardware configuration */

#include "cros_cbi.h"
#include "gpio/gpio_int.h"
#include "hooks.h"
#include "pujjoniru_sub_board.h"
#include "task.h"
...
test_export_static enum pujjoniru_sub_board_type pujjoniru_cached_sub_board =
	PUJJONIRU_SB_UNKNOWN;
/*
...
enum pujjoniru_sub_board_type pujjoniru_get_sb_type(void)
{
	 ...
	 * Return cached value.
	 */
	if (pujjoniru_cached_sub_board != PUJJONIRU_SB_UNKNOWN)
		return pujjoniru_cached_sub_board;

	pujjoniru_cached_sub_board = PUJJONIRU_SB_NONE; /* Defaults to none */
	...
	switch (val) {
	case FW_SUB_BOARD_1:
		pujjoniru_cached_sub_board = PUJJONIRU_SB_C;
 ...
 */
static void pujjoniru_subboard_config(void)
{
	enum pujjoniru_sub_board_type sb = pujjoniru_get_sb_type();

	if (sb != PUJJONIRU_SB_C) {
		/* Port doesn't exist, doesn't need muxing */
		USB_MUX_ENABLE_ALTERNATIVE(usb_mux_chain_1_no_mux);
	}
}
DECLARE_HOOK(HOOK_INIT, pujjoniru_subboard_config, HOOK_PRIO_POST_FIRST);
