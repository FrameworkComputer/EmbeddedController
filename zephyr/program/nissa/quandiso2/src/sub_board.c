...
/* Quandiso2 sub-board hardware configuration */
...
test_export_static enum quandiso2_sub_board_type quandiso2_cached_sub_board =
	QUANDISO2_SB_UNKNOWN;
/*
...
enum quandiso2_sub_board_type quandiso2_get_sb_type(void)
{
	 ...
	 * Return cached value.
	 */
	if (quandiso2_cached_sub_board != QUANDISO2_SB_UNKNOWN)
		return quandiso2_cached_sub_board;

	quandiso2_cached_sub_board = QUANDISO2_SB_C_A; /* Defaults to 1A1C */
	...
	switch (val) {
	case FW_SUB_BOARD_1:
		quandiso2_cached_sub_board = QUANDISO2_SB_ABSENT;
	...
	case FW_SUB_BOARD_2:
		...
		break;
	case FW_SUB_BOARD_3:
		quandiso2_cached_sub_board = QUANDISO2_SB_LTE;
		LOG_INF("SubBoard: Only LTE");
		break;
	case FW_SUB_BOARD_4:
		quandiso2_cached_sub_board = QUANDISO2_SB_C_LTE;
...
{
	switch (quandiso2_get_sb_type()) {
	case QUANDISO2_SB_ABSENT:
	case QUANDISO2_SB_LTE:
 ...
 */
static void quandiso2_subboard_config(void)
{
	switch (quandiso2_get_sb_type()) {
	case QUANDISO2_SB_ABSENT:
	case QUANDISO2_SB_LTE:
...
DECLARE_HOOK(HOOK_INIT, quandiso2_subboard_config, HOOK_PRIO_POST_FIRST);
