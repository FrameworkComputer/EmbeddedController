...
/* Rull sub-board hardware configuration */

#include "cros_cbi.h"
#include "gpio/gpio_int.h"
#include "hooks.h"
#include "rull_sub_board.h"
#include "task.h"
...
test_export_static enum rull_sub_board_type rull_cached_sub_board =
	RULL_SB_UNKNOWN;
/*
...
enum rull_sub_board_type rull_get_sb_type(void)
{
	 ...
	 * Return cached value.
	 */
	if (rull_cached_sub_board != RULL_SB_UNKNOWN)
		return rull_cached_sub_board;

	rull_cached_sub_board = RULL_SB_NONE; /* Defaults to none */
	...
	switch (val) {
	case FW_SUB_BOARD_1:
		rull_cached_sub_board = RULL_SB_C;
 ...
 */
static void rull_subboard_config(void)
{
	enum rull_sub_board_type sb = rull_get_sb_type();
...
DECLARE_HOOK(HOOK_INIT, rull_subboard_config, HOOK_PRIO_POST_FIRST);
