...
static const struct board_led_pwm_dt_channel battery_left_amber_led =
	BOARD_LED_PWM_DT_CHANNEL_INITIALIZER(
		DT_NODELABEL(pwm_battery_left_amber_led));

static const struct board_led_pwm_dt_channel battery_left_white_led =
	BOARD_LED_PWM_DT_CHANNEL_INITIALIZER(
		DT_NODELABEL(pwm_battery_left_white_led));

static const struct board_led_pwm_dt_channel battery_right_amber_led =
	BOARD_LED_PWM_DT_CHANNEL_INITIALIZER(
		DT_NODELABEL(pwm_battery_right_amber_led));

static const struct board_led_pwm_dt_channel battery_right_white_led =
	BOARD_LED_PWM_DT_CHANNEL_INITIALIZER(
		DT_NODELABEL(pwm_battery_right_white_led));
...
static void led_set_left_battery_duty(enum led_color color, int duty)
{
	if (color == LED_WHITE) {
		led_pwm_set_duty(&battery_left_white_led, duty);
		led_pwm_set_duty(&battery_left_amber_led, 0);
	} else if (color == LED_AMBER) {
		led_pwm_set_duty(&battery_left_white_led, 0);
		led_pwm_set_duty(&battery_left_amber_led, duty);
	} else {
		led_pwm_set_duty(&battery_left_white_led, 0);
		led_pwm_set_duty(&battery_left_amber_led, 0);
	}
...
static void led_set_right_battery_duty(enum led_color color, int duty)
{
	if (color == LED_WHITE) {
		led_pwm_set_duty(&battery_right_white_led, duty);
		led_pwm_set_duty(&battery_right_amber_led, 0);
	} else if (color == LED_AMBER) {
		led_pwm_set_duty(&battery_right_white_led, 0);
		led_pwm_set_duty(&battery_right_amber_led, duty);
	} else {
		led_pwm_set_duty(&battery_right_white_led, 0);
		led_pwm_set_duty(&battery_right_amber_led, 0);
	}
...
static void led_auto_set_battery_color(enum led_color color, int duty)
{
...
int led_set_brightness(enum ec_led_id led_id, const uint8_t *brightness)
{
	...
	case EC_LED_ID_POWER_LED:
		if (brightness[EC_LED_COLOR_BLUE] != 0)
			rv = led_set_color_power(LED_BLUE,
...
static bool is_battery_led_auto_control(void)
{
	return led_auto_control_is_enabled(EC_LED_ID_LEFT_LED) &&
...
static void led_set_battery(void)
{
	...
	battery_ticks++;
	if (is_battery_led_auto_control()) {
		switch (led_pwr_get_state()) {
		case LED_PWRS_CHARGE:
			/* Always indicate when charging, even in suspend. */
			led_auto_set_battery_color(LED_AMBER, 100);
			break;
		case LED_PWRS_DISCHARGE:
			if (charge_get_percent() <= BATTERY_LEVEL_CRITICAL &&
				...
				led_auto_set_battery_color(LED_OFF, 0);
			}
			break;
		case LED_PWRS_ERROR:
			led_auto_set_battery_color(
				(battery_ticks & 0x1) ? LED_AMBER : LED_OFF,
				(battery_ticks & 0x1) ? 100 : 0);
			break;
		case LED_PWRS_CHARGE_NEAR_FULL:
			led_auto_set_battery_color(LED_WHITE, 100);
			break;
	...
	if ((led_pwr_get_state() != LED_PWRS_DISCHARGE) ||
	    (!is_battery_led_auto_control() &&
	     (led_pwr_get_state() == LED_PWRS_DISCHARGE))) {
		battery_low_triggeied = 0;
		battery_critical_triggeied = 0;
...
static void battery_set_pwm_led_tick(void)
{
	...
	if (is_battery_led_auto_control()) {
		...
		led_auto_set_battery_color(batt_led_pulse.color,
					   batt_led_pulse.duty);
	}
	...
	if (new_mode != MODE_NO_CHANGE && new_mode != current_mode) {
		...
		case MODE_NORMAL:
			if (led_auto_control_is_enabled(EC_LED_ID_POWER_LED))
				led_set_color_power(LED_BLUE, 100);
			break;
		case MODE_SUSPEND:
			PWR_LED_CONFIG_TICK(PWR_LED_PULSE_TICK_US, LED_BLUE);
			break;
		case MODE_OFF:
			if (led_auto_control_is_enabled(EC_LED_ID_POWER_LED))
