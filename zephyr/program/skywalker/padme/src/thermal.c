...
static void typec_temperature_policy_deferred(void)
{
	...
	/* Check Type-C temperature every second and suspend PD after 3
	 * consecutive overheat readings to avoid transient spikes.
	 */
	if (typec_temp_c >= 100) {
		if (typec_policy)
			typec_policy_cyc = 3;
...
static void average_tempature(void)
{
	 ...
	 *    0      55       70      80     85
	 */
	temp_sensor_read(TEMP_SENSOR_ID_BY_DEV(DT_NODELABEL(temp_charger2)),
			 &charger_temp);
	...
	if (thermals[4]) {
		if (chipset_state != POWER_S0) {
			...
			if (temperature_increase) {
				...
				if (charger_temp_ave < 55 &&
				    temp_zone >= TEMP_ZONE_1)
					clear_remaining_array(time, ROW_NUM, 1,
							      0);
				else if (charger_temp_ave < 70 &&
					 temp_zone >= TEMP_ZONE_2)
					clear_remaining_array(time, ROW_NUM, 1,
							      1);
