...
#define AMP_ENABLE_DELAY_MS (1200 * USEC_PER_MSEC)

void amp_enable(void)
{
	int state;

	state = gpio_pin_get_dt(GPIO_DT_FROM_NODELABEL(gpio_plt_rst));
	LOG_INF("kaladin: amp_enable_DEFERRED: gpio_plt_rst: %d", state);
	if (state) {
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_amp_mute_l), 1);
		LOG_INF("gpio_amp_mute_l: %d",
			gpio_pin_get_dt(
				GPIO_DT_FROM_NODELABEL(gpio_amp_mute_l)));
	}
}
DECLARE_DEFERRED(amp_enable);

void plt_rst_interrupt(enum gpio_signal signal)
{
	int state;
	...
	if (state) {
		hook_call_deferred(&amp_enable_data, AMP_ENABLE_DELAY_MS);
	} else {
		gpio_pin_set_dt(GPIO_DT_FROM_NODELABEL(gpio_amp_mute_l), 0);
	}

	LOG_INF("kaladin: %s: %d", __func__, state);
}

static void plt_rst_enable_init(void)
{
	gpio_enable_dt_interrupt(GPIO_INT_FROM_NODELABEL(int_plt_rst));
}
DECLARE_HOOK(HOOK_INIT, plt_rst_enable_init, HOOK_PRIO_DEFAULT);
