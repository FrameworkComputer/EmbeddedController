...
#include "egis_bep_bio_alg.h"

#include <zephyr/logging/log.h>

#include <fingerprint/fingerprint_alg.h>

LOG_MODULE_REGISTER(egis_egis660_alg, LOG_LEVEL_INF);

/*
 * This file contains structures required by EGIS library (algorithm) and
 * implementation of algorithm API
 */

/* EGIS BEP algorithm private data. */
struct egis_bep_data {
	uint32_t enroll_ctx;
};

BUILD_ASSERT(sizeof(((struct egis_bep_data *)0)->enroll_ctx) ==
		     FP_ALGORITHM_ENROLLMENT_SIZE,
	     "Wrong enroll_ctx size");

static int
egis_bep_algorithm_init(const struct fingerprint_algorithm *const alg)
{
	int rc;

	if (!IS_ENABLED(CONFIG_HAVE_EGIS660_PRIVATE_ALGORITHM)) {
...
static int
egis_bep_algorithm_exit(const struct fingerprint_algorithm *const alg)
{
	int rc;
...
static int egis_bep_enroll_start(const struct fingerprint_algorithm *const alg)
{
	struct egis_bep_data *data = (struct egis_bep_data *)alg->data;
...
static int egis_bep_enroll_step(const struct fingerprint_algorithm *const alg,
...
static int egis_bep_enroll_finish(const struct fingerprint_algorithm *const alg,
				  void *templ)
{
	struct egis_bep_data *data = (struct egis_bep_data *)alg->data;
	bio_enrollment_t bio_enroll = &data->enroll_ctx;
...
static int egis_bep_match(const struct fingerprint_algorithm *const alg,
...
const struct fingerprint_algorithm_api egis_bep_api = {
	.init = egis_bep_algorithm_init,
	.exit = egis_bep_algorithm_exit,
	.enroll_start = egis_bep_enroll_start,
	.enroll_step = egis_bep_enroll_step,
	.enroll_finish = egis_bep_enroll_finish,
	.match = egis_bep_match,
};

static struct egis_bep_data egis_bep_data;
FINGERPRINT_ALGORITHM_DEFINE(egis_bep_algorithm, &egis_bep_data, &egis_bep_api);
