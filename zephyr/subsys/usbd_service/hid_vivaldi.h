...
#define KEYBOARD_TOP_ROW_FEATURE_DESC                                    \
	0x06, 0xd1, 0xff, /* Usage Page (Google) */                      \
		...
		HID_USAGE_MIN8(0x01),                                    \
		HID_USAGE_MAX8(CONFIG_USBD_KEYBOARD_NUM_TOP_ROW_KEYS),   \
		HID_REPORT_COUNT(CONFIG_USBD_KEYBOARD_NUM_TOP_ROW_KEYS), \
		HID_REPORT_SIZE(32), HID_FEATURE(0x03), HID_END_COLLECTION

int32_t get_vivaldi_feature_report(const uint16_t req_len, uint8_t *data);
