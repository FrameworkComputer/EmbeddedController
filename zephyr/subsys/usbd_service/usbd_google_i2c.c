...
UDC_BUF_POOL_DEFINE(gi2c_pool, 32, GOOGLE_EP_FS_MPS,
...
static void gi2c_output_handler(struct k_work *work)
{
...
static ALWAYS_INLINE void gi2c_out_cb(struct usbd_class_data *const c_data,
				      struct net_buf *const buf)
{
	struct net_buf *out_buf;

	out_buf = gi2c_buf_alloc(google_get_out_ep(c_data));
	if (!out_buf) {
...
static int usbd_gi2c_init(struct usbd_class_data *const c_data)
{
	...
	data->c_data = c_data;
	k_work_init(&data->output_work, gi2c_output_handler);

	k_thread_create(&rx_thread_data, rx_thread_stack,
			K_KERNEL_STACK_SIZEOF(rx_thread_stack), gi2c_rx_thread,
			(void *)c_data, NULL, NULL,
...
void i2c_usb__stream_written(struct consumer const *consumer, size_t count)
{
	...
	if (queue_is_empty(consumer->queue)) {
		LOG_ERR("i2c_usb consumer queue is empty");
		return;
	}
	...
	do {
		...
		if (!buf) {
			LOG_ERR("i2c_usb failed to allocate tx buffer");
			return;
		}
