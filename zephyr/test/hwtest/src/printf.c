...
static const char init_value = 0x5E;
static const char *no_bytes_touched = NULL;

static const char err_str[] = "ERROR";
static char output[1024];

ZTEST_SUITE(printf, NULL, NULL, NULL, NULL, NULL);

static void run(int expect_ret, const char *expect, bool output_null,
		size_t size_limit, const char *format, va_list args)
{
	...
	zassert_true(expect_size <= sizeof(output));
	zassert_true(expect_size <= size_limit);
	...
	zassert_mem_equal(output, expect, expect_size);
	for (int i = 0; i < sizeof(output) - expect_size; i++) {
		zassert_true((&output[expect_size])[i] == init_value);
	}

	if (rv >= 0) {
		zassert_true(rv == expect_size - 1);
		zassert_true(EC_SUCCESS == expect_ret);
	} else {
		zassert_true(rv == -expect_ret);
	}
}

static void expect_success_crec(const char *expect, const char *format, ...)
{
	va_list args;

	va_start(args, format);
	run(EC_SUCCESS, expect, false, sizeof(output), format, args);
	va_end(args);
}

static void expect_success_std(const char *format, ...)
{
	...
	va_start(args, format);
	rv = vsnprintf(expect_std, sizeof(expect_std), format, args);
	va_end(args);

	va_start(args, format);
	run(EC_SUCCESS, expect_std, false, sizeof(output), format, args);
	va_end(args);
}

static void expect_success(const char *expect, const char *format, ...)
{
	char expect_std[1024];
	int ret;
	va_list args;
	 ...
	 */
	va_start(args, format);
	ret = vsnprintf(expect_std, sizeof(expect_std), format, args);
	va_end(args);

	ccprintf("standard='%.*s'\n", 30, expect_std);
	zassert_true(strcmp(expect_std, expect) == 0);
	zassert_true(ret >= 0);
}

static void expect(int expect_ret, const char *expect, bool output_null,
		   size_t size_limit, const char *format, ...)
{
	va_list args;

	va_start(args, format);
	run(expect_ret, expect, output_null, size_limit, format, args);
	va_end(args);
}

ZTEST(printf, test_vsnprintf_args)
{
	expect_success("", "");
	expect_success("a", "a");
	 ...
	 */
	expect(/* expect an invalid args error */
	       EC_ERROR_INVAL, no_bytes_touched,
	       /* given 0 as output size limit */
	       false, 0, "");
	expect(/* expect an overflow error */
	       EC_ERROR_OVERFLOW, "",
...
ZTEST(printf, test_vsnprintf_int)
{
	expect_success("123", "%d", 123);
	expect_success("-123", "%d", -123);
	expect_success("+123", "%+d", 123);
	expect_success("-123", "%+d", -123);
	expect_success("123", "%-d", 123);
	...
	expect_success("  123", "%5d", 123);
	expect_success(" +123", "%+5d", 123);
	expect_success("00123", "%05d", 123);
	expect_success("00123", "%005d", 123);

	/* Precision or width larger than buffer should fail. */
	expect(EC_ERROR_OVERFLOW, "  1", false, 4, "%5d", 123);
	expect(EC_ERROR_OVERFLOW, "   ", false, 4, "%10d", 123);
	expect(EC_ERROR_OVERFLOW, "123", false, 4, "%-10d", 123);
	expect(EC_ERROR_OVERFLOW, "0.0", false, 4, "%.10d", 123);

	expect_success("+0123", "%+05d", 123);
	expect_success("+0123", "%+005d", 123);
	expect_success("  123", "%*d", 5, 123);
	expect_success(" +123", "%+*d", 5, 123);
	expect_success("00123", "%0*d", 5, 123);
	expect_success("00123", "%00*d", 5, 123);
	expect_success("+0123", "%+0*d", 5, 123);
	expect_success("+0123", "%+00*d", 5, 123);

	expect_success("123  ", "%-5d", 123);
	expect_success("+123 ", "%-+5d", 123);
	expect_success("+123 ", "%+-5d", 123);
	expect_success("123  ", "%-05d", 123);
	expect_success("123  ", "%-005d", 123);
	expect_success("+123 ", "%-+05d", 123);
	expect_success("+123 ", "%-+005d", 123);

	expect_success("123", "%u", 123);
	expect_success("4294967295", "%u", -1);
	expect_success("18446744073709551615", "%llu", (uint64_t)-1);

	expect_success("0", "%x", 0);
	expect_success("0", "%X", 0);
	expect_success("5e", "%x", 0X5E);
	expect_success("5E", "%X", 0X5E);
}

ZTEST(printf, test_vsnprintf_fixed_point)
{
	/*
	 * Fixed point formatting is a cros-ec
	 * deviation from the standard.
	 */
	expect_success_crec("0.00123", "%.5d", 123);
	expect_success_crec("12.3", "%2.1d", 123);
	expect_success_crec("0.00123", "%.5d", 123);
	expect_success_crec("+0.00123", "%+.5d", 123);
	expect_success_crec("0.00123", "%7.5d", 123);
	expect_success_crec("  0.00123", "%9.5d", 123);
	expect_success_crec(" +0.00123", "%+9.5d", 123);
	expect_success_crec("+0000.123", "%+09.3d", 123);
	expect_success_crec("-0000.123", "%+09.3d", -123);
}

ZTEST(printf, test_vsnprintf_long32_enabled)
{
...
static void test_vsnprintf_32bit_long_supported(void)
{
	...
	expect_success(long_min_str, "%ld", long_min);
	expect_success(long_min_hexstr, "%lx", long_min);
	expect_success(long_max_str, "%ld", long_max);
	expect_success(long_max_hexstr, "%lx", long_max);
	expect_success(ulong_max_str, "%lu", ulong_max);
	expect_success(ulong_max_hexstr, "%lx", ulong_max);
	expect_success(long_max_str, "%ld", long_max);

	expect_success(" +123", "%+*ld", 5, 123);
	expect_success("00000123", "%08lu", 123);
	expect_success("131415", "%d%lu%d", 13, 14L, 15);
	 ...
	 */
	expect_success_crec("123", "%i", 123);
	expect_success_crec("123", "%li", 123);
}

static void test_vsnprintf_64bit_long_supported(void)
{
...
ZTEST(printf, test_vsnprintf_32bit_long)
{
	/*
	...
	if (IS_ENABLED(CONFIG_PRINTF_LONG_IS_32BITS))
		test_vsnprintf_32bit_long_supported();
	else if (sizeof(long) == sizeof(uint64_t))
		test_vsnprintf_64bit_long_supported();
	else
		test_vsnprintf_long_not_supported();
}

ZTEST(printf, test_vsnprintf_pointers)
{
	 ...
	 */
	expect_success_crec("55005e00", "%p", ptr);
}

ZTEST(printf, test_vsnprintf_chars)
{
	expect_success("a", "%c", 'a');
	expect_success("*", "%c", '*');
}

ZTEST(printf, test_vsnprintf_strings)
{
	char fmt[100];
	int ret;

	expect_success("abc", "%s", "abc");
	expect_success("  abc", "%5s", "abc");
	expect_success("abc", "%0s", "abc");
	expect_success("abc  ", "%-5s", "abc");
	expect_success("abc", "%*s", 0, "abc");
	expect_success("a", "%.1s", "abc");
	expect_success("a", "%.*s", 1, "abc");
	expect_success("", "%.0s", "abc");
	expect_success("", "%.*s", 0, "abc");
	expect_success("abc", "%.4s", "abc");
	...
	for (int width = 0; width < 6; ++width) {
		for (int prec = 0; prec < 6; ++prec) {
			ret = snprintf(fmt, sizeof(fmt), "%%%d.%ds", width,
				       prec);
			zassert_true(ret >= 0);
			expect_success_std(fmt, "abc");
		}
	}
	 ...
	 */
	expect_success_crec("", "%.0s", (char *)1);
}

ZTEST(printf, test_vsnprintf_combined)
{
	expect_success("abc", "%c%s", 'a', "bc");
	expect_success("12\tbc", "%d\t%s", 12, "bc");
}

ZTEST(printf, test_uint64_to_str)
{
	...
	zassert_mem_equal(str, "0", sizeof("0"));

	str = uint64_to_str(buf, sizeof(buf), /*val=*/UINT64_MAX,
			    /*precision=*/-1, /*base=*/10,
			    /*uppercase=*/false);
	zassert_mem_equal(str, "18446744073709551615",
	...
	str = uint64_to_str(buf, sizeof(buf), /*val=*/UINT64_MAX,
			    /*precision=*/-1, /*base=*/16,
			    /*uppercase=*/false);
	zassert_mem_equal(str, "ffffffffffffffff", sizeof("fffffffffffffff"));
...
ZTEST(printf, test_snprintf_timestamp)
{
...
ZTEST(printf, test_snprintf_hex_buffer)
{
	...
	const char buf_ff[5] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
	char str_buf[5];
	int rv;
