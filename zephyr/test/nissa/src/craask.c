...
ZTEST(craask, test_set_active_charge_port_none)
{
	uint16_t reg;
...
ZTEST(craask, test_set_active_charge_port_invalid_port)
{
	zassert_equal(board_set_active_charge_port(4), EC_ERROR_INVAL,
		      "port 4 doesn't exist, should return error");
}

ZTEST(craask, test_set_active_charge_port_currently_sourcing)
{
...
ZTEST(craask, test_set_active_charge_port)
{
	uint16_t reg;
...
ZTEST(craask, test_set_active_charge_port_enable_fail)
{
	i2c_common_emul_set_write_fail_reg(
		emul_tcpci_generic_get_i2c_common_data(TCPC0),
		TCPC_REG_COMMAND);
...
ZTEST(craask, test_set_active_charge_port_disable_fail)
{
...
ZTEST(craask, test_tcpc_get_alert_status)
{
...
ZTEST(craask, test_set_source_current_limit)
{
...
ZTEST(craask, test_pd_set_power_supply_ready)
{
	uint16_t reg;
...
ZTEST(craask, test_reset_pd_mcu)
{
	/* Doesn't do anything */
	board_reset_pd_mcu();
}

ZTEST(craask, test_process_pd_alert)
{
	const struct gpio_dt_spec *c0_int =
		GPIO_DT_FROM_NODELABEL(gpio_usb_c0_int_odl);
	const struct gpio_dt_spec *c1_int =
...
ZTEST(craask, test_touch_enable)
{
