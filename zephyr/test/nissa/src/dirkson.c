...
ZTEST_SUITE(dirkson, NULL, NULL, test_before, NULL, NULL);
...
ZTEST(dirkson, test_extpower_is_present)
{
	/* AC is always present */
	zassert_true(extpower_is_present());
}

ZTEST(dirkson, test_ppc_interrupt)
{
...
ZTEST(dirkson, test_board_vbus_source_enabled)
{
	/* check C0 */
...
ZTEST(dirkson, test_pd_power_supply_reset)
{
...
ZTEST(dirkson, test_pd_set_power_supply_ready)
{
...
ZTEST(dirkson, test_pd_snk_is_vbus_provided)
{
...
ZTEST(dirkson, test_led_shutdown)
{
...
ZTEST(dirkson, test_led_suspend)
{
...
ZTEST(dirkson, test_led_brightness_range)
{
...
ZTEST(dirkson, test_board_get_vbus_voltage)
{
...
ZTEST(dirkson, test_adp_connect_interrupt)
{
...
ZTEST(dirkson, test_board_set_active_charge_port)
{
	const struct gpio_dt_spec *bj_adp_present =
		GPIO_DT_FROM_NODELABEL(gpio_bj_adp_present);
...
ZTEST(dirkson, test_board_charge_manager_init)
{
	const struct gpio_dt_spec *bj_adp_present =
		GPIO_DT_FROM_NODELABEL(gpio_bj_adp_present);
...
ZTEST(dirkson, test_power_monitor)
{
...
ZTEST(dirkson, test_board_is_power_good)
{
	const struct gpio_dt_spec *bj_adp_present =
		GPIO_DT_FROM_NODELABEL(gpio_bj_adp_present);
...
ZTEST(dirkson, test_usba_retimer_init)
{
...
ZTEST(dirkson, test_typec_ilim_control)
{
