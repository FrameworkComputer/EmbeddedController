...
ZTEST_SUITE(domika, NULL, NULL, test_before, NULL, NULL);
...
ZTEST(domika, test_extpower_is_present)
{
	/* Errors are not-OK */
...
ZTEST(domika, test_board_check_extpower)
{
...
ZTEST(domika, test_board_hibernate)
{
	board_hibernate();
	zassert_equal(sm5803_hibernate_fake.call_count, 2);
}

ZTEST(domika, test_board_vconn_control)
{
...
ZTEST(domika, test_pd_check_vbus_level)
{
...
ZTEST(domika, test_chargers_suspend)
{
...
ZTEST(domika, test_set_active_charge_port)
{
...
ZTEST(domika, test_tcpc_get_alert_status)
{
...
ZTEST(domika, test_pd_power_supply_reset)
{
...
ZTEST(domika, test_pd_set_power_supply_ready)
{
...
ZTEST(domika, test_typec_set_source_current_limit)
{
...
ZTEST(domika, test_typec_set_sink_current_limit)
{
...
ZTEST(domika, test_c0_interrupt)
{
...
ZTEST(domika, test_c1_interrupt)
{
...
ZTEST(domika, test_board_handle_initial_typec_irq)
{
	const struct gpio_dt_spec *const c0_irq =
		GPIO_DT_FROM_NODELABEL(gpio_usb_c0_int_odl);
...
ZTEST(domika, test_board_process_pd_alert)
{
	const struct gpio_dt_spec *const c1_irq =
		GPIO_DT_FROM_NODELABEL(gpio_usb_c1_int_odl);
...
ZTEST(domika, test_pd_snk_is_vbus_provided)
{
...
ZTEST(domika, test_board_vivaldi_keybd_idx)
{
...
ZTEST(domika, test_kb_layout_init)
{
	cros_cbi_get_fw_config_fake.custom_fake =
...
ZTEST(domika, test_kb_layout_init_cbi_error)
{
...
ZTEST(domika, test_fan_cbi_error)
{
	int flags;
