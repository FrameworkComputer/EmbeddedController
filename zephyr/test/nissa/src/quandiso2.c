...
ZTEST_SUITE(quandiso2, NULL, NULL, test_before, NULL, NULL);
...
ZTEST(quandiso2, test_extpower_is_present)
{
	/* Errors are not-OK */
...
ZTEST(quandiso2, test_board_check_extpower)
{
...
ZTEST(quandiso2, test_board_hibernate)
{
	board_hibernate();
	zassert_equal(sm5803_hibernate_fake.call_count, 2);
}

ZTEST(quandiso2, test_board_vconn_control)
{
...
ZTEST(quandiso2, test_pd_check_vbus_level)
{
...
ZTEST(quandiso2, test_chargers_suspend)
{
...
ZTEST(quandiso2, test_set_active_charge_port)
{
...
ZTEST(quandiso2, test_tcpc_get_alert_status)
{
...
ZTEST(quandiso2, test_pd_power_supply_reset)
{
...
ZTEST(quandiso2, test_pd_set_power_supply_ready)
{
...
ZTEST(quandiso2, test_typec_set_source_current_limit)
{
...
ZTEST(quandiso2, test_typec_set_sink_current_limit)
{
...
ZTEST(quandiso2, test_c0_interrupt)
{
...
ZTEST(quandiso2, test_c1_interrupt)
{
...
ZTEST(quandiso2, test_board_handle_initial_typec_irq)
{
	const struct gpio_dt_spec *const c0_irq =
		GPIO_DT_FROM_NODELABEL(gpio_usb_c0_int_odl);
...
ZTEST(quandiso2, test_board_process_pd_alert)
{
	const struct gpio_dt_spec *const c1_irq =
		GPIO_DT_FROM_NODELABEL(gpio_usb_c1_int_odl);
...
ZTEST(quandiso2, test_pd_snk_is_vbus_provided)
{
...
ZTEST(quandiso2, test_board_vivaldi_keybd_idx)
{
...
ZTEST(quandiso2, test_kb_layout_init)
{
...
ZTEST(quandiso2, test_kb_layout_init_cbi_error)
{
...
ZTEST(quandiso2, test_fan_present)
{
	int flags;
...
ZTEST(quandiso2, test_fan_absent)
{
	int flags;
...
ZTEST(quandiso2, test_fan_cbi_error)
{
	int flags;
...
enum quandiso2_sub_board_type quandiso2_get_sb_type(void);
...
ZTEST(quandiso2, test_db_with_c)
{
	cros_cbi_get_fw_config_fake.custom_fake =
		get_fake_sub_board_fw_config_field;
	/* Reset cached global state. */
	...
	/* Set the sub-board, reported configuration is correct. */
	set_fw_config_value(FW_SUB_BOARD_2);
	zassert_equal(quandiso2_get_sb_type(), QUANDISO2_SB_C_A);
...
ZTEST(quandiso2, test_db_with_lte_only)
{
	cros_cbi_get_fw_config_fake.custom_fake =
		get_fake_sub_board_fw_config_field;
	/* Reset cached global state. */
	...
	zassert_equal(quandiso2_get_sb_type(), QUANDISO2_SB_LTE);
	zassert_equal(board_get_usb_pd_port_count(), 1);
...
ZTEST(quandiso2, test_db_with_c_lte)
{
	cros_cbi_get_fw_config_fake.custom_fake =
		get_fake_sub_board_fw_config_field;
	/* Reset cached global state. */
	...
	/* Set the sub-board, reported configuration is correct. */
	set_fw_config_value(FW_SUB_BOARD_4);
	zassert_equal(quandiso2_get_sb_type(), QUANDISO2_SB_C_LTE);
	zassert_equal(board_get_usb_pd_port_count(), 2);
...
ZTEST(quandiso2, test_cbi_error)
{
	/*
...
ZTEST(quandiso2, test_fan_table)
{
...
ZTEST(quandiso2, test_convertible)
{
...
ZTEST(quandiso2, test_clamshell)
{
	const struct device *tablet_mode_gpio = DEVICE_DT_GET(
		DT_GPIO_CTLR(DT_NODELABEL(gpio_tablet_mode_l), gpios));
	const gpio_port_pins_t tablet_mode_pin =
...
ZTEST(quandiso2, test_alt_sensor_lid_bma422)
{
