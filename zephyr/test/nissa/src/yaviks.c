...
ZTEST(yaviks, test_extpower_is_present)
{
	/* Errors are not-OK */
...
ZTEST(yaviks, test_board_check_extpower)
{
...
ZTEST(yaviks, test_board_hibernate)
{
	board_hibernate();
	zassert_equal(sm5803_hibernate_fake.call_count, 2);
}

ZTEST(yaviks, test_board_vconn_control)
{
...
ZTEST(yaviks, test_pd_check_vbus_level)
{
...
ZTEST(yaviks, test_chargers_suspend)
{
...
ZTEST(yaviks, test_set_active_charge_port)
{
...
ZTEST(yaviks, test_tcpc_get_alert_status)
{
...
ZTEST(yaviks, test_pd_power_supply_reset)
{
...
ZTEST(yaviks, test_pd_set_power_supply_ready)
{
...
ZTEST(yaviks, test_typec_set_source_current_limit)
{
...
ZTEST(yaviks, test_typec_set_sink_current_limit)
{
...
ZTEST(yaviks, test_c0_interrupt)
{
...
ZTEST(yaviks, test_c1_interrupt)
{
...
ZTEST(yaviks, test_board_handle_initial_typec_irq)
{
	const struct gpio_dt_spec *const c0_irq =
		GPIO_DT_FROM_NODELABEL(gpio_usb_c0_int_odl);
...
ZTEST(yaviks, test_board_process_pd_alert)
{
	const struct gpio_dt_spec *const c1_irq =
		GPIO_DT_FROM_NODELABEL(gpio_usb_c1_int_odl);
...
ZTEST(yaviks, test_pd_snk_is_vbus_provided)
{
...
cros_cbi_get_fw_config_kb_blight(enum cbi_fw_config_field_id field_id,
				 uint32_t *value)
{
	if (field_id != FW_KB_BACKLIGHT)
		return -EINVAL;

	switch (kb_blight) {
	case 0:
		*value = FW_KB_BACKLIGHT_ON;
		break;
	case 1:
...
ZTEST(yaviks, test_board_vivaldi_keybd_idx)
{
...
ZTEST(yaviks, test_kb_layout_init_cbi_error)
{
...
ZTEST(yaviks, test_fan_cbi_error)
{
	int flags;
...
ZTEST(yaviks, test_db_with_c)
{
	...
	/* Set the sub-board, reported configuration is correct. */
	set_fw_config_value(FW_SUB_BOARD_2);
	zassert_equal(yaviks_get_sb_type(), YAVIKS_SB_C_A);
...
static int cros_cbi_get_fw_config_fan_type(enum cbi_fw_config_field_id field_id,
					   uint32_t *value)
{
	if (field_id != FAN_TYPE)
		return -EINVAL;
...
ZTEST(yaviks, test_fan_type_2)
{
	...
	board_override_fan_control(0, temp);
	zassert_equal(fan_get_rpm_mode(0), 1);
	zassert_equal(fan_get_rpm_target(0), 3800);
