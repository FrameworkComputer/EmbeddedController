...
ZTEST_SUITE(pdc_runtime_config_one_pdc, NULL, NULL, NULL, NULL, NULL);

ZTEST_USER(pdc_runtime_config_one_pdc, test_board_count)
{
	zassert_equal(1, pdc_power_mgmt_get_usb_pd_port_count());
}

ZTEST_USER(pdc_runtime_config_one_pdc, test_board_get_drivers)
{
...
ZTEST_USER(pdc_runtime_config_one_pdc, test_board_get_state)
{
...
ZTEST_USER(pdc_runtime_config_one_pdc, test_board_get_info)
{
	...
	zassert_equal(-ERANGE, rv, "Expected -ERANGE but got %d", rv);
}
